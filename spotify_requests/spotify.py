...
USER_TOP_ARTISTS_ENDPOINT = "{}/{}/{}".format(USER_PROFILE_ENDPOINT, 'top','artists') 
USER_TOP_TRACKS_ENDPOINT = "{}/{}/{}".format(USER_PROFILE_ENDPOINT, 'top','tracks')  
...
GET_PLAYLIST_TRACKS_ENDPOINT = "{}/{}".format(SPOTIFY_API_URL, 'playlists')
...
def get_users_top_artists(auth_header):
    url = USER_TOP_ARTISTS_ENDPOINT
    resp = requests.get(url, headers=auth_header)
    return resp.json()

# https://developer.spotify.com/web-api/get-users-top-artists-and-tracks/
def get_users_top_tracks(auth_header):
    url = USER_TOP_TRACKS_ENDPOINT
    resp = requests.get(url, headers=auth_header)
    return resp.json()
...
def get_playlist(auth_header, id):
    url = "{}/{id}".format(GET_PLAYLIST_TRACKS_ENDPOINT, id=id)
    resp = requests.get(url, headers=auth_header)
    return resp.json()

def get_playlist_tracks(auth_header, playlist_id):
    url = "{}/{id}/tracks".format(GET_PLAYLIST_TRACKS_ENDPOINT, id=playlist_id)
    resp = requests.get(url, headers=auth_header)
    return resp.json()
